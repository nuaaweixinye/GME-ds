...
`SessionPromptParams.resumeIfExists` explicitly asks the server to resume persisted state for `sessionId` before creating it. Omission or `false` creates directly. A missing persistence record falls back to creation; corruption, unsupported format versions, active write ownership, and every other persistence failure reject the prompt without creating a replacement session.
