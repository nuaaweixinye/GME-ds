...
`HarnessClient` 会在运行时进程的整个生命周期内保留已发现的子 agent 祖先关系。在 `Session.run()` 期间，`RunResult.notifications` 与 `on_notification` 按协议顺序接收根会话和已知后代的通知。`RunResult.events` 只包含根会话事件，因此后代输出不会替换根响应。`DeepSeekHarness.run()`、`Session.run()` 与底层 `session_prompt()` 都接受针对具名会话的 `resume_if_exists=True`：持久历史存在时运行时会打开它，仅在没有已存会话时创建会话。默认行为仍是仅创建；持久状态损坏、版本不受支持或已被占用时恢复会被拒绝。底层方法会立即返回已排队消息的 id；绕过 `Session.run()` 的调用方自行负责后续活动边界。

所选 home 保存 profile、插件与每个 profile 自有的持久资源。完整 `sdk` profile 使用其中的凭据、设置与会话存储；`sdk-minimal` 只使用自己的 JSONL 会话存储。需要隔离这些资源时应使用新的 home；独立工作应使用新的 session id。在同一运行时内复用 session id 会延续其活跃对话；该运行时关闭后，应复用同一个 home 并传入 `resume_if_exists=True`，以在另一个进程中恢复其持久对话。
