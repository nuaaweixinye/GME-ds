...
| `@deepseek-ai/dsh-tool-weknora` | `knowledge_search` | `ctx.tools`、`ctx.systemPrompt`、调用时的 `ctx.credentials` 或启动环境 | `tool/call`、`tool/result` | - | knowledge_search 是一个 opt-in WeKnora 检索工具。目录使用 dummy base URL 和知识库 ID，因为凭据与网络访问只在工具执行时解析。 |
...
```json
{
  ...
  "properties": {
    ...
    "after": {
      "type": "integer",
      "description": "Return events with IDs greater than this value."
    },
    "offset": {
      "type": "integer",
      "description": "Character offset for the next page of the same report."
    }
  },
  "required": [
    "resource"
...
搜索已配置的 WeKnora 知识库，返回相关片段。回答依赖私有或项目知识的问题前使用此工具。
...
来源：[`packages/web/tool-weknora/src/index.ts`](../packages/web/tool-weknora/src/index.ts)

knowledge_search 是一个 opt-in WeKnora 检索工具。目录使用 dummy base URL 和知识库 ID，因为凭据与网络访问只在工具执行时解析。
