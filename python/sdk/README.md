...
`HarnessClient` retains discovered subagent ancestry for the runtime process lifetime. During `Session.run()`, `RunResult.notifications` and `on_notification` receive the root session and known descendants in wire order. `RunResult.events` contains root-session events only, so descendant output cannot replace the root response. `DeepSeekHarness.run()`, `Session.run()`, and the low-level `session_prompt()` accept `resume_if_exists=True` for a named session: the runtime opens persisted history when present and creates only when no stored session exists. The default remains create-only; corrupt, unsupported, or already-owned persisted state rejects recovery. The low-level method returns the queued message id immediately, and callers that bypass `Session.run()` own the later activity boundary.

The selected home stores profiles, plugins, and every profile-owned durable resource. The full `sdk` profile uses its credentials, settings, and session stores; `sdk-minimal` uses only its JSONL session store. Use a fresh home when those resources must be isolated, and a fresh session id for independent work. Reusing a session id within one runtime continues its live conversation; after that runtime closes, reuse the same home and pass `resume_if_exists=True` to recover its persisted conversation in another process.
