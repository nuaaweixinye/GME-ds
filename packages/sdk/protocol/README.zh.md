...
`SessionPromptParams.resumeIfExists` 显式要求服务器先为 `sessionId` 恢复持久化状态，再决定是否创建。省略或设为 `false` 时直接创建。仅持久化记录不存在时才回退到创建；记录损坏、格式版本不受支持、写所有权被占用及任何其他持久化失败都会拒绝提示词，且不会创建替代会话。
