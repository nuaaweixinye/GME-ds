...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  if (clientBuildValue('DSH_CLIENT_BUILD_PROFILE') === 'official') {
    ...
    expect(screen.queryByText('GME')).toBeNull()
  } else {
    expect(document.querySelector('image[href="/gme-logo.png"]')).not.toBeNull()
    ...
    expect(screen.getAllByText('GME').length).toBeGreaterThanOrEqual(2)
