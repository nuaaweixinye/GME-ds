...
插件在首次使用时为每个 `sessionId` 打开一个 agent。带 `resumeIfExists: true` 的提示词会先恢复持久化会话，且仅在持久化记录不存在时创建。记录损坏、格式版本不受支持、写所有权被占用及任何其他恢复失败都会拒绝提示词，且不会创建替代会话；省略或设为 `false` 时直接创建。已注册的模型适配器赢得路由；尚无适配器负责的 `deepseek-official` 路由会挂载 DeepSeek 适配器，任何其他尚无适配器负责的提供方都会导致初始化失败。初始化成功前，所选适配器会解析确切模型与可选推理强度。
